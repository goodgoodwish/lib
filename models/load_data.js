...
var Postgres = require('../libs/Postgres.js')
...
//   var author = Author.add(authordetail);
...
async function genreCreate(name, cb) {
    ...
    res = await Genre.add(name, '/catalog/genre/' + name)
    genre = res.rows;
    console.log(genre)
    console.log('New Genre: ' + genre[0].name);
    genres.push(genre[0].name)
    cb(null, genre);
}

async function genreDel(cb) {
    await Genre.delAll()
    cb(null, {});
}
...
function(err, results) {
    if (err) {
        ...
        console.log('BOOKInstances: '+genres);
